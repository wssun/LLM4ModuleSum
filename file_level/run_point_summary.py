...
def main():
    ...
    target_folder = '../result/file-level/GPT4/method3_communtiy/json_result'
    ...
    for json_filename in json_files:
        ...
        for file_path, communities in data.items():
            ...
            community_code.append(
                f'To facilitate better summarization, the class {classname} has been divided into several code fragments. Here are the names of these code fragments and their summaries:')
            counter = 1
            for community_id, elements in communities.items():
                community_code.append(f"code fragment {counter}: {elements}\n")
                counter += 1

            code.append("\n".join(community_code))

        print(f'File {json_filename} has {len(code)} code fragments.')

        output_csv_path = os.path.join('../result/file-level/GPT4/method3_communtiy/final_result', f'{os.path.splitext(json_filename)[0]}_zero_shot.csv')
